import requests
import random
from functions.third_apps.errors import unable_connect_api
from settings import RECIPE_URL, RECIPE_APP_ID, RECIPE_APP_KEY
...
class RecipeAPI:
    ...
    def send_recipe(self, update, context):
        ...
        try:
            ...
            update.message.reply_text("Please send the recipe you are looking for...")
            return
        try:
            ...
            update.message.reply_text("Something went wrong with API credentials :c")
            return

        update.message.reply_text(self.recipe)
        update.message.reply_text(self.ingredients)
